 ...
 */
int8_t MPU9250_drv_init()
{
	int8_t res = MPU9250_OK;
    uint8_t val = 0x00;

    /* MPU9250 reset & initial */
    for (int i = 0; i < 100; i++) {
        I2C_Read(MPU9250_I2C_ADDRESS, MPU9250_REG_WHO_AM_I, &val, 1);
        if (val == 0x71) {
            break;
        }
        HAL_Delay(100);
    ...
    if (val != 0x71) {
        return MPU9250_NOT_FOUND;
    }

	uint8_t MPU9250InitConf[][2] = {
		...
		{MPU9250_REG_USER_CTRL,     0x00},  /* I2C master mode */
		{0xff,                      0xff}
	};

    for (int i = 0; MPU9250InitConf[i][0] != 0xff; i++) {
    	val = I2C_Write(MPU9250_I2C_ADDRESS,MPU9250InitConf[i][0], &MPU9250InitConf[i][1],1);
		if (val){
    		return MPU9250_INIT_ERROR;
    	}
    	HAL_Delay(100);
    }


        I2C_Read(AK8963_I2C_ADDR, AK8963_REG_WIA, &val, 1);
        if (val != 0x48) {
        	return AK8963_INIT_ERROR;
        }
        uint8_t conf = 0x01;
		/* Reset */
        I2C_Write(AK8963_I2C_ADDR,AK8963_REG_CNTL2,&conf,1);
		HAL_Delay(100);  /* 10ms */

        /* read calibration data */
        I2C_Read(AK8963_I2C_ADDR,AK8963_REG_ASAX,&magnetometer_calib,sizeof(magnetometer_calib));
		HAL_Delay(100);  /* 10ms */
        /* 16bit periodical mode. (8Hz) */
        conf = 0x12;
        I2C_Write(AK8963_I2C_ADDR,AK8963_REG_CNTL1,&conf,1);
		HAL_Delay(100);  /* 10ms */
    ...
    for (int i = 0; init_conf[i][0] != 0xff; i++) {
    	if (I2C_Write(MPU9250_I2C_ADDRESS,init_conf[i][0], init_conf[i][1],1)){
    		return MPU9250_INIT_ERROR;
    	}
    }
...
bool MPU9250_drv_start_maesure(MPU9250_BIT_GYRO_FS_SEL gyro_fs, MPU9250_BIT_ACCEL_FS_SEL accel_fs, MPU9250_BIT_DLPF_CFG dlpf_cfg, MPU9250_BIT_A_DLPFCFG a_dlpfcfg)
{
    ...
    for (int i = 0; init_conf[i][0] != 0xff; i++) {
    	val = I2C_Write(MPU9250_I2C_ADDRESS,init_conf[i][0], &init_conf[i][1],1);
		if (val){
			return MPU9250_INIT_ERROR;
		}
		HAL_Delay(100);
...
bool MPU9250_drv_stop_maesure(void)
{
    ...
    uint8_t conf = 0x3f;
	I2C_Write(MPU9250_I2C_ADDRESS,MPU9250_REG_PWR_MGMT_2, &conf,1);
	HAL_Delay(100);
...
bool MPU9250_drv_read_gyro(MPU9250_gyro_val *gyro_val)
{
	...
	I2C_Read(MPU9250_I2C_ADDRESS,MPU9250_REG_GYRO_XOUT_HL,vals,sizeof(vals));
...
bool MPU9250_drv_read_accel(MPU9250_accel_val *accel_val)
{
	...
	I2C_Read(MPU9250_I2C_ADDRESS,MPU9250_REG_ACCEL_XOUT_HL,vals,sizeof(vals));
...
bool MPU9250_drv_read_magnetometer(MPU9250_magnetometer_val *magnetometer_val)
{
    ...
    HAL_Delay(10);
	I2C_Read(AK8963_I2C_ADDR,AK8963_REG_HX_LH,vals,sizeof(vals));

    /* RAW data */
    magnetometer_val->raw_x = ((uint16_t)vals[1] << 8) | vals[0];
    magnetometer_val->raw_y = ((uint16_t)vals[3] << 8) | vals[2];
    magnetometer_val->raw_z = ((uint16_t)vals[5] << 8) | vals[4];
    /* Real data */
